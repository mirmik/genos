...
static inline void hlist_add_next(struct hlist_node *list,
                                  struct hlist_node **hnext)
{
    list->pprev = hnext;
    list->next = *hnext;
    if (list->next)
        list->next->pprev = &(list->next);
    *hnext = list;
}
...
#define hlist_entry(ptr, type, member) mcast_out(ptr, type, member)
...
#define hlist_for_each_entry(pos, head, member)                                \
    for (pos = hlist_first_entry(head, __typeof__(*pos), member);              \
         &pos->member != 0; pos = hlist_next_entry(pos, member))
