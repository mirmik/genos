...
// Стандартная таблица CRC32 (полином 0xEDB88320, IEEE 802.3)
...
uint32_t crc32_ccitt_std(const void *data, uint32_t length, uint32_t crc_init)
{
...
//
// Compute CCITT CRC32 as in STM32 MCU family, short table based
// Not thread-safe
// For continuous calculation set initCrc to previous returned crc32 value
// Trailing zeros for data not multiples of 4 bytes
//
uint32_t crc32_stm32(const void *data, uint32_t length, uint32_t initCrc)
{
...
{
    return crc32_stm32(data, length, crc_init);
}