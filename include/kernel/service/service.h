...
namespace genos {
	...
	public:
		virtual int srvs_send_query (struct gstack* stack, qid_t rqid);
		virtual int srvs_add_query (struct query *q);
		virtual int srvs_receive_query (qid_t q, struct gstack** ppstack, qid_t *retqid);
		virtual int srvs_reply_answer (struct query *q);
		virtual int srvs_receive_answer (struct query *q);
	};
