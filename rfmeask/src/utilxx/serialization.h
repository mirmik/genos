...
static bool is_need_reverse(uint8_t order)
{
    ...
    if (order == PROTOORD_BIG)
        return true;
    return false;
#elif __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
    if (order == PROTOORD_LITTLE)
        return true;
    return false;
#else
...
{
    if (is_need_reverse(flag))
        memcpy_swap(dst, src, sz);
    else
        memcpy(dst, src, sz);
}
...
{
    return obj.serialization(buf, flag);
}
...
// serialsize template
template <typename T> size_t serialSize(const T &ref)
{
                                   ...
                                   uint8_t flag)
{
    if (maxlen < 1)
        return 0;
    __memcpy(ptr, &ref, 1, flag);
    return 1;
}

static inline size_t serialization(char *ptr, const uint16_t &ref,
                                   size_t maxlen, uint8_t flag)
{
    if (maxlen < 2)
        return 0;
    __memcpy(ptr, &ref, 2, flag);
    return 2;
}

static inline size_t serialization(char *ptr, const uint32_t &ref,
                                   size_t maxlen, uint8_t flag)
{
    if (maxlen < 4)
        return 0;
    __memcpy(ptr, &ref, 4, flag);
    return 4;
}

static inline size_t serialization(char *ptr, const uint64_t &ref,
                                   size_t maxlen, uint8_t flag)
{
    if (maxlen < 8)
        return 0;
    __memcpy(ptr, &ref, 8, flag);
    return 8;
}
...
static inline size_t plainSerialization(char *dst, void *src, uint16_t len,
                                        size_t maxlen)
{
    if (maxlen < len)
        return 0;
...
static inline size_t plainDeserialization(char *dst, void *src, uint16_t len,
                                          size_t maxlen)
{
    if (maxlen < len)
        return 0;
...
public:
    ...
    {
        if (deserialSize<T>(m_data) > m_size)
            return -1;
