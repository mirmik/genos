...
static inline void stm32_timer_set_prescaler(TIM_TypeDef * tim, uint16_t prescaler)
{
	tim->PSC = prescaler - 1;
}
...
static inline
void stm32_timer_ovf_irq_enable(TIM_TypeDef * tim, uint8_t en) 
{
	bits_lvl(tim->DIER, TIM_DIER_UIE, en);
} 

static inline
void stm32_timer_drop_ovf_flag(TIM_TypeDef * tim) 
{
