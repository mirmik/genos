...
static inline
uint32_t stm32_adc_status(struct stm32_adc_regs * regs) 
{
	return regs->SR;
}
...
static inline 
void stm32_adc_regular_channel_sequence_length(struct stm32_adc_regs * regs, int total) 
{
	bits_assign_bias(regs->SQR1, 0b1111, total - 1, 20);
}

static inline
void stm32_adc_regular_channel_rank(struct stm32_adc_regs * regs, int ch, int rank) 
{
	uint8_t bias;
