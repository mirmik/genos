...
static inline
int stm32l4_usart_putc(usart_regs_t * usart, char c)
{
	usart->TDR = c;
	return 1;
}
...
static inline
int stm32l4_usart_getc(usart_regs_t * usart)
{
	return usart->RDR;
}

static inline
int stm32l4_usart_room(usart_regs_t * usart)
{
	return usart->ISR & (1 << 7); //TXNE byte
}

static inline
int stm32l4_usart_avail(usart_regs_t * usart)
{
	return usart->ISR & (1 << 5); //RXNE byte
}

static inline
int stm32l4_rxirq_status(usart_regs_t * usart) 
{
	return usart->ISR & USART_ISR_RXNE;
}

static inline
int stm32l4_txirq_status(usart_regs_t * usart) 
{
	return usart->ISR & USART_ISR_TXE;
}

static inline
int stm32l4_tcirq_status(usart_regs_t * usart) 
{
