...
void i2c_irq_handler(void* arg) {
	arch::i2c_automate* i2c = (arch::i2c_automate*) arg;
	uint8_t code = TWSR;
	...
	switch(code & 0xF8)	{ // Отсекаем биты прескалера
		...
		case 0x08: { // Старт был, а затем мы:
			if ( i2c->type == arch::i2c_automate::i2c_sarp ) {	// В зависимости от режима
				//dprln("ADDR+R"); 
		...
		case 0x10: { // Повторный старт был, а затем мы
			if ( i2c->type == arch::i2c_automate::i2c_sawsarp ) {	// В зависимости от режима
				TWDR = i2c->target_address | 0x01;		// Шлем Addr+R
			}
			else {									// Или 
		...
		case 0x18: { // Был послан SLA+W получили ACK, а затем:
			if( i2c->type == arch::i2c_automate::i2c_sawp || i2c->type == arch::i2c_automate::i2c_sawsarp) { // В зависимости от режима
		...
		case 0x28: { // Байт данных послали, получили ACK!  (если sawp - это был байт данных. если sawsarp - байт адреса страницы)
			// А дальше: 
			if( i2c->type == arch::i2c_automate::i2c_sawp ) {// В зависимости от режима
				if (i2c->it == i2c->sendbuf.size()) { // Если был байт данных последний											
					...
					i2c->operation_finish_handler();												// И выходим в обработку стопа
				}
				else {
			...
			if( i2c->type == arch::i2c_automate::i2c_sawsarp )	{					// В другом режиме мы
				if (i2c->it == i2c->sendbuf.size()) { // Если был байт данных последний											
					i2c->it = 0;
	...
	/*
		...
		case 0x58: { // Вот мы взяли последний байт, сказали NACK слейв обиделся и отпал. 
			...
			i2c->operation_finish_handler();														// Отработали точку выхода
			break;
		}
/*	
/		IC  Slave ============================================================================
