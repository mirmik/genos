...
static inline void usart_regs_enable_rx(struct usart_regs* regs, int en)
{
	bits_lvl(regs->ucsr_b, (1 << RXEN0), en);
}

static inline void usart_regs_enable_tx(struct usart_regs* regs, int en)
{
	bits_lvl(regs->ucsr_b, (1 << TXEN0), en);
}

static inline void usart_regs_rxirq(struct usart_regs* regs, int en)
{
	bits_lvl(regs->ucsr_b, (1 << RXCIE0), en);
}

static inline void usart_regs_txirq(struct usart_regs* regs, int en)
{
	bits_lvl(regs->ucsr_b, (1 << UDRIE0), en);
}

static inline void usart_regs_tcirq(struct usart_regs* regs, int en)
{
