...
namespace httplib
{
    ...
    using ContentReceiverWithProgress =
        std::function<bool(const char *data,
                           size_t data_length,
                           uint64_t offset,
    ...
    public:
        ...
        using MultipartReader = std::function<bool(
    ...
    public:
        ...
        Server &
        set_file_extension_and_mimetype_mapping(const std::string &ext,
    ...
    private:
        ...
        bool handle_file_request(const Request &req,
                                 ...
                                 bool head = false);
        bool
        dispatch_request(Request &req, Response &res, const Handlers &handlers);
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
        void
        set_ca_cert_path(const std::string &ca_cert_file_path,
    ...
    protected:
        ...
        // concurrently with a DIFFERENT thread sending requests using that
        // socket.
    ...
    private:
        ...
        bool
        read_response_line(Stream &strm, const Request &req, Response &res);
    ...
    public:
        ...
        SSLServer(const std::function<bool(SSL_CTX &ssl_ctx)>
                      &setup_ssl_ctx_callback);
    ...
    namespace detail
    {
        ...
        public:
            ...
            typedef std::function<bool(const char *data, size_t data_len)>
                Callback;
        ...
        // NOTE: until the read size reaches `fixed_buffer_size`, use
        // `fixed_buffer` to store data. The call can set memory on stack for
        // performance.
    ...
    namespace detail
    {
        ...
        {
            static const auto lookup = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklm"
        ...
        inline std::string encode_query_param(const std::string &value)
        {
            ...
            for (auto c : value)
            {
                if (std::isalnum(static_cast<uint8_t>(c)) || c == '-' ||
                    c == '_' || c == '.' || c == '!' || c == '~' || c == '*' ||
                    c == '\'' || c == '(' || c == ')')
                {
                    escaped << c;
        ...
        inline void stream_line_reader::append(char c)
        {
            ...
            else
            {
                if (glowable_buffer_.empty())
                {
                    ...
                    glowable_buffer_.assign(fixed_buffer_,
        ...
        wait_until_socket_is_ready(socket_t sock, time_t sec, time_t usec)
        {
            ...
            auto poll_res =
                handle_EINTR([&]() { return poll(&pfd_read, 1, timeout); });
        ...
        inline bool is_socket_alive(socket_t sock)
        {
            ...
            char buf[1];
            return detail::read_socket(sock, &buf[0], sizeof(buf), MSG_PEEK) >
                   0;
        }
        ...
        template <typename T>
        inline bool
        process_server_socket_core(const std::atomic<socket_t> &svr_sock,
                                   socket_t sock,
            ...
            for (auto rp = result; rp; rp = rp->ai_next)
            {
                ...
                auto sock = WSASocketW(rp->ai_family,
                                       ...
                                       WSA_FLAG_NO_HANDLE_INHERIT |
                                           WSA_FLAG_OVERLAPPED);
                /**
                 * Since the WSA_FLAG_NO_HANDLE_INHERIT is only supported on
                 * Windows 7 SP1 and above the socket creation fails on older
                 * Windows Systems.
                 *
                 ...
                 *
                 * WSA_FLAG_NO_HANDLE_INHERIT:
                 * This flag is supported on Windows 7 with SP1, Windows Server
        ...
        inline bool bind_ip_address(socket_t sock, const std::string &host)
        {
            ...
            if (getaddrinfo_static(host.c_str(), "0", &hints, &result))
            {
                return false;
            }
...
#define USE_IF2IP
            ...
            for (auto ifa = ifap; ifa; ifa = ifa->ifa_next)
            {
                     ...
                     ifa->ifa_addr->sa_family == address_family))
                {
                    if (ifa->ifa_addr->sa_family == AF_INET)
                    {
                        ...
                        if (inet_ntop(
                                AF_INET, &sa->sin_addr, buf, INET_ADDRSTRLEN))
                        {
                            freeifaddrs(ifap);
                    ...
                    {
                        auto sa = reinterpret_cast<struct sockaddr_in6 *>(
                            ifa->ifa_addr);
            ...
            auto sock = create_socket(
                ...
                [&](socket_t sock2, struct addrinfo &ai) -> bool
                {
                    ...
                    if (ret < 0)
                    {
                        ...
                        error =
                            wait_until_socket_is_ready(sock2,
                                           ...
                                           int &port)
        {
            ...
            {
                port = ntohs(reinterpret_cast<const struct sockaddr_in *>(&addr)
                                 ->sin_port);
            }
            else if (addr.ss_family == AF_INET6)
            {
                port =
                    ntohs(reinterpret_cast<const struct sockaddr_in6 *>(&addr)
                              ->sin6_port);
            }
            else
            {
        ...
        inline EncodingType encoding_type(const Request &req,
                                          const Response &res)
        {
            auto ret = detail::can_compress_content_type(
        ...
        inline gzip_decompressor::gzip_decompressor()
        {
            ...
            // value to ensure that any gzip stream can be decoded. The offset
            // of 32 specifies that the stream type should be automatically
            // detected either gzip or deflate.
        ...
        inline bool read_headers(Stream &strm, Headers &headers)
        {
            ...
            for (;;)
            {
                ...
                auto end = line_reader.ptr() + line_reader.size() -
                           line_terminator_len;
        ...
        {
            return !strcasecmp(
                get_header_value(headers, "Transfer-Encoding", 0, ""),
                "chunked");
        }
                                      ...
                                      U callback)
        {
            if (decompress)
            {
                ...
                if (decompressor)
                {
                    ...
                    {
                        ContentReceiverWithProgress out = [&](const char *buf,
                                                              size_t n,
                                                              uint64_t off,
                                                              uint64_t len)
                        {
                          ...
                          bool decompress)
        {
            return prepare_content_receiver(
                ...
                [&](const ContentReceiverWithProgress &out)
                {
                    ...
                    {
                        auto len = get_header_value<uint64_t>(x.headers,
                                                              "Content-Length");
                        ...
                        {
                            ret = read_content_with_length(
                                strm, len, std::move(progress), out);
                        }
                    }
            ...
            data_sink.write = [&](const char *d, size_t l) -> bool
            {
                if (ok)
                {
                            ...
                            }))
                    {
                        if (!payload.empty())
                        {
                            // Emit chunked response header and footer for each
                            // chunk
        ...
        inline void parse_query_text(const std::string &s, Params &params)
        {
            ...
            split(s.data(),
                  ...
                  [&](const char *b, const char *e)
                  {
                      ...
                      if (!key.empty())
                      {
                          params.emplace(decode_url(key, true),
                                         decode_url(val, true));
                      }
                  });
        }
                        ...
                        while (pos < buf_size())
                        {
                            ...
                            if (start_with_case_ignore(header, header_name))
                            {
                                file_.content_type = trim_copy(
                                    header.substr(header_name.size()));
                            }
                            else
                            {
                                std::smatch m;
        ...
        {
            static const char data[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabc"
                                       "defghijklmnopqrstuvwxyz";

            // std::random_device might actually be deterministic on some
            // platforms, but due to lack of support in the c++ standard
            // library, doing better requires either some ugly hacks or breaking
            // portability.
        ...
        inline std::string make_content_range_header_field(
            size_t offset, size_t length, size_t content_length)
        {
                                           ...
                                           Content content)
        {
            for (size_t i = 0; i < req.ranges.size(); i++)
            {
                ...
                auto offsets =
                    get_range_offset_and_length(req, res.body.size(), i);
                auto offset = offsets.first;
                auto length = offsets.second;

                ctoken("Content-Range: ");
                stoken(make_content_range_header_field(
        ...
        inline size_t
        get_multipart_ranges_data_length(const Request &req,
                                         Response &res,
                                         const std::string &boundary,
                                         const std::string &content_type)
        {
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
        inline std::string message_digest(const std::string &s,
                                          const EVP_MD *algo)
        {
            auto context =
        ...
        {
            auto hStore =
                CertOpenSystemStoreW((HCRYPTPROV_LEGACY)NULL, L"ROOT");
                   ...
                   nullptr)
            {
                ...
                auto x509 =
                    d2i_X509(NULL, &encoded_cert, pContext->cbCertEncoded);
                if (x509)
                {
            ...
            bool is_proxy = false)
        {
            ...
            auto opaque =
                (auth.find("opaque") != auth.end()) ? auth.at("opaque") : "";
        ...
        inline bool
        parse_www_authenticate(const Response &res,
                               std::map<std::string, std::string> &auth,
                               bool is_proxy)
        {
            auto auth_key =
                is_proxy ? "Proxy-Authenticate" : "WWW-Authenticate";
                ...
                if (pos != std::string::npos)
                {
                    ...
                    else if (type == "Digest")
                    {
                        ...
                        for (auto i = beg; i != std::sregex_iterator(); ++i)
                        {
                            auto m = *i;
                            auto key =
                                s.substr(static_cast<size_t>(m.position(1)),
                          ...
                          std::vector<std::string> &addrs)
    {
        ...
        for (auto rp = result; rp; rp = rp->ai_next)
        {
            ...
            std::string ip;
            int dummy = -1;
            if (detail::get_remote_ip_and_port(
    ...
    {
        auto field =
            "Basic " + detail::base64_encode(username + ":" + password);
        auto key = is_proxy ? "Proxy-Authorization" : "Authorization";
    ...
    namespace detail
    {
        ...
        {
            return select_read(sock_, read_timeout_sec_, read_timeout_usec_) >
                   0;
        }
        ...
        {
            return select_write(
                       sock_, write_timeout_sec_, write_timeout_usec_) > 0;
        }
        ...
        inline ssize_t SocketStream::read(char *ptr, size_t size)
        {
            ...
            if (read_buff_off_ < read_buff_content_size_)
            {
                ...
                {
                    memcpy(ptr,
                           read_buff_.data() + read_buff_off_,
                           remaining_size);
                                            ...
                                            bool need_apply_ranges)
    {
        ...
        if (req.method != "HEAD")
        {
            ...
            {
                if (write_content_with_provider(
...
#ifdef CPPHTTPLIB_BROTLI_SUPPORT
                    compressor =
                        detail::make_unique<detail::brotli_compressor>();
#endif
                }
                else
                {
    ...
    inline bool
    Server::read_content_core(Stream &strm,
                              Request &req,
                              Response &res,
                              ContentReceiver receiver,
                              MultipartContentHeader mulitpart_header,
                      ...
                      uint64_t /*len*/)
            {
                ...
                while (pos < n) {
                  ...
                  auto ret = multipart_form_data_parser.parse(
                      buf + pos, read_size, multipart_receiver,
                mulitpart_header); if (!ret) { return false; } pos += read_size;
                }
                return true;
                */
                return multipart_form_data_parser.parse(
                    buf, n, multipart_receiver, mulitpart_header);
            };
        }
        else
        {
            out = [receiver](const char *buf,
                             size_t n,
                             uint64_t /*off*/,
    ...
    Server::handle_file_request(const Request &req, Response &res, bool head)
    {
        for (const auto &entry : base_dirs_)
        {
            // Prefix match
            if (!req.path.compare(
                    0, entry.mount_point.size(), entry.mount_point))
            {
                std::string sub_path =
    ...
    inline socket_t
    Server::create_server_socket(const std::string &host,
                                 int port,
                                 int socket_flags,
                                 SocketOptions socket_options) const
    {
            ...
            while (svr_sock_ != INVALID_SOCKET)
            {
                ...
                if (sock == INVALID_SOCKET)
                {
                    if (errno == EMFILE)
                    {
                        ...
                        // been reached. Try to accept new connections after a
                        // short sleep.
                ...
                }
                {
                    ...
                    setsockopt(
                        sock, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv, sizeof(tv));
#endif
                }
                                     ...
                                     std::string &boundary)
    {
        ...
        if (res.body.empty())
        {
            if (res.content_length_ > 0)
            {
                ...
                else if (req.ranges.size() == 1)
                {
                    ...
                    auto content_range =
                        detail::make_content_range_header_field(
        ...
        else
        {
            ...
            {
                std::string data;
                if (detail::make_multipart_ranges_data(
                        req, res, boundary, content_type, data))
                {
                    res.body.swap(data);
                }
                else
                            ...
                            const std::function<void(Request &)> &setup_request)
    {
        ...
        if (routed)
        {
            ...
            return write_response_with_content(
                strm, close_connection, req, res);
        }
        else
        {
            if (res.status == -1)
    ...
    {
        // If there are any requests in flight from threads other than us, then
        // it's a thread-unsafe race because individual ssl* objects are not
        // thread-safe.
    ...
    {
        std::lock_guard<std::recursive_mutex> request_mutex_guard(
            request_mutex_);

        {
            std::lock_guard<std::mutex> guard(socket_mutex_);

            // Set this to false immediately - if it ever gets set to true by
            // the end of the request, we know another thread instructed us to
            // close the socket.
            ...
            if (socket_.is_open())
            {
                ...
                if (!is_alive)
                {
                    // Attempt to avoid sigpipe by shutting down nongracefully
                    // if it seems like the other side has already closed the
                    // connection Also, there cannot be any requests in flight
                    // from other threads since we locked request_mutex_, so
                    // safe to close everything immediately
            ...
            // Mark the current socket as being in use so that it cannot be
            // closed by anyone else while this request is ongoing, even though
            // we will be releasing the mutex.
    ...
    inline Result ClientImpl::send_(Request &&req)
    {
        ...
        return Result{
            ret ? std::move(res) : nullptr, error, std::move(req.headers)};
    }
    ...
    inline bool ClientImpl::redirect(Request &req, Response &res, Error &error)
    {
        ...
        auto location =
            detail::decode_url(res.get_header_value("location"), true);
        if (location.empty())
        {
            return false;
        }
        ...
        else
        {
            if (next_scheme == "https")
            {
                ...
                return detail::redirect(
                    cli, req, res, next_path, location, error);
#else
                return false;
#endif
            }
                                                        ...
                                                        Error &error)
    {
        ...
        if (req.is_chunked_content_provider_)
        {
            ...
            return detail::write_content_chunked(strm,
                                          ...
                                          Error &error)
    {
            ...
            !proxy_basic_auth_password_.empty())
        {
            ...
            {
                req.headers.insert(
                    make_basic_authentication_header(proxy_basic_auth_username_,
        ...
        Error &error)
    {
        ...
        if (compress_ && !content_provider_without_length)
        {
            ...
            if (content_provider)
            {
                ...
                {
                    if (!content_provider(
                            offset, content_length - offset, data_sink))
                    {
                        error = Error::Canceled;
                        return nullptr;
                    }
                                            ...
                                            Error &error)
    {
             ...
             res.reason != "Connection established"))
        {
            ...
            // correct for this to be safe. Maybe a code refactor (such as
            // moving this out to the send function and getting rid of the
            // recursiveness of the mutex) could make this more obvious.

            // This is safe to call because process_request is only called by
            // handle_request which is only called by send, which locks the
            // request mutex during the process. It would be a bug to call it
            // from a different thread since it's a thread-safety issue to do
            // these things to the socket if another thread is using the socket.
            std::lock_guard<std::mutex> guard(socket_mutex_);
            shutdown_ssl(socket_, true);
            shutdown_socket(socket_);
            close_socket(socket_);
    ...
    {
        return Get(
            path, Headers(), nullptr, std::move(content_receiver), nullptr);
    }
                                  ...
                                  Progress progress)
    {
        ...
        {
            return Get(
                path, headers, response_handler, content_receiver, progress);
        }

        std::string path_with_query = append_query_params(path, params);
        return Get(path_with_query.c_str(),
                   headers,
                   response_handler,
                   content_receiver,
                   progress);
    ...
    {
        return Post(
            path, headers, items, detail::make_multipart_data_boundary());
    }
    inline Result ClientImpl::Post(const std::string &path,
                                   const Headers &headers,
                                   const MultipartFormDataItems &items,
    ...
    {
        return Patch(
            path, Headers(), std::move(content_provider), content_type);
    }
    ...
    inline void ClientImpl::stop()
    {
        ...
        // If there is anything ongoing right now, the ONLY thread-safe thing we
        // can do is to shutdown_socket, so that threads using this socket
        // suddenly discover they can't read/write any more and error out.
        // Everything else (closing the socket, shutting ssl down) is unsafe
        // because these actions are not thread-safe.
        if (socket_requests_in_flight_ > 0)
        {
            shutdown_socket(socket_);

            // Aside from that, we set a flag for the socket to be closed when
            // we're done.
            socket_should_be_closed_when_request_is_done_ = true;
            return;
        }

        // Otherwise, sitll holding the mutex, we can shut everything down
        // ourselves
        shutdown_ssl(socket_, true);
        shutdown_socket(socket_);
        close_socket(socket_);
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    inline void
    ClientImpl::set_ca_cert_path(const std::string &ca_cert_file_path,
 ...
 */
    ...
    namespace detail
    {
        ...
        inline void
        ssl_delete(std::mutex &ctx_mutex, SSL *ssl, bool shutdown_gracefully)
        {
            // sometimes we may want to skip this to try to avoid SIGPIPE if we
            // know the remote has closed the network connection Note that it is
            // not always possible to avoid SIGPIPE, this is merely a
            // best-efforts.
        ...
        template <typename T>
        inline bool
        process_server_socket_ssl(const std::atomic<socket_t> &svr_sock,
                                  SSL *ssl,
                                  socket_t sock,
                                  size_t keep_alive_max_count,
                                  time_t keep_alive_timeout_sec,
        ...
        public:
            SSLThreadLocks()
            {
                openSSL_locks_ = std::make_shared<std::vector<std::mutex>>(
                    CRYPTO_num_locks());
        ...
        private:
            static void locking_callback(int mode,
                                         int type,
                                         const char * /*file*/,
                                         int /*line*/)
            {
        ...
        {
            return detail::select_read(
                       sock_, read_timeout_sec_, read_timeout_usec_) > 0;
        }

        inline bool SSLSocketStream::is_writable() const
        {
            return detail::select_write(
                       sock_, write_timeout_sec_, write_timeout_usec_) > 0;
        }
                    ...
                    while (--n >= 0 && err == SSL_ERROR_WANT_WRITE)
                    {
                        ...
                        if (is_writable())
                        {
                            std::this_thread::sleep_for(
                                std::chrono::milliseconds(1));
                            ret = SSL_write(
                                ssl_, ptr, static_cast<int>(handle_size));
                            if (ret >= 0)
                            {
                                return ret;
                            }
                                ...
                                const char *private_key_password)
    {
        ...
        if (ctx_)
        {
            ...
            // add default password callback before opening encrypted private
            // key
            if (private_key_password != nullptr &&
                (private_key_password[0] != '\0'))
            {
                SSL_CTX_set_default_passwd_cb_userdata(
                    ctx_, (char *)private_key_password);
            }

            if (SSL_CTX_use_certificate_chain_file(ctx_, cert_path) != 1 ||
                SSL_CTX_use_PrivateKey_file(
                    ctx_, private_key_path, SSL_FILETYPE_PEM) != 1)
            {
                SSL_CTX_free(ctx_);
                ctx_ = nullptr;
            }
    ...
    inline bool SSLServer::process_and_close_socket(socket_t sock)
    {
        ...
        if (ssl)
        {
            ...
            // Shutdown gracefully if the result seemed successful,
            // non-gracefully if the connection appeared to be closed.
    ...
    {
        return is_valid() &&
               ClientImpl::create_and_connect_socket(socket, error);
    }
                            ...
                            [&](Stream &strm)
                            {
                                ...
                                req3.headers.insert(
                                    detail::make_digest_authentication_header(
                                        ...
                                        true));
                                return process_request(
                                    strm, req3, res3, false, error);
                            }))
                    {
                        // Thread-safe to close everything because we are
                        // assuming there are no requests in flight
                        shutdown_ssl(socket, true);
                        shutdown_socket(socket);
                        close_socket(socket);
                        success = false;
    ...
    inline bool SSLClient::load_certs()
    {
        ...
        std::call_once(
            ...
            [&]()
            {
                ...
                {
#ifdef _WIN32
                    detail::load_system_certs_on_windows(
    ...
    inline void SSLClient::shutdown_ssl(Socket &socket,
                                        bool shutdown_gracefully)
    {
    ...
    SSLClient::verify_host_with_subject_alt_name(X509 *server_cert) const
    {
        ...
        auto alt_names =
            static_cast<const struct stack_st_GENERAL_NAME *>(X509_get_ext_d2i(
                server_cert, NID_subject_alt_name, nullptr, nullptr));
    ...
    {
        return cli_->Get(
            path, std::move(content_receiver), std::move(progress));
    }
    inline Result Client::Get(const std::string &path,
                              const Headers &headers,
                              ContentReceiver content_receiver,
