...
int stm32_usart_room(USART_TypeDef *usart)
{
    return usart->ISR & USART_ISR_TXFE;
}
...
void stm32_usart_set_parity(USART_TypeDef *regs, char parity)
{
    ...
    case 'e':
        bits_assign(regs->CR1, USART_CR1_PCE | USART_CR1_PS, USART_CR1_PCE);
        break;
...
{
    if (en)
        bits_set(regs->CR1, USART_CR1_UE) else bits_clr(regs->CR1,
                                                        USART_CR1_UE);
}
...
{
    if (en)
        bits_set(regs->CR1, USART_CR1_RE) else bits_clr(regs->CR1,
                                                        USART_CR1_RE);
}
...
{
    if (en)
        bits_set(regs->CR1, USART_CR1_TE) else bits_clr(regs->CR1,
                                                        USART_CR1_TE);
}
...
{
    if (en)
        bits_set(regs->CR1, USART_CR1_RXNEIE) else bits_clr(regs->CR1,
                                                            USART_CR1_RXNEIE);
}
...
{
    if (en)
        bits_set(regs->CR1, USART_CR1_TXEIE) else bits_clr(regs->CR1,
                                                           USART_CR1_TXEIE);
}
