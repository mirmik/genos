...
unsigned stm32_init_pll_clocking(struct stm32_pll_settings *s)
{
    ...
    // RCC->CFGR |= RCC_CFGR_HPRE_DIV16 // RCC_CFGR_HPRE_DIV1
    ...
    RCC->PLLCFGR =
        (RCC_PLLCFGR_PLLSRC_HSE | (RCC_PLLCFGR_PLLM & (s->Mkoeff << 0)) |
...
#elif defined(STM32G4XX) || defined(STM32L4XX) || defined(STM32F7XX) || defined(STM32H7XX)
...
unsigned stm32_init_pll_clocking(struct stm32_pll_settings *s)
{
    ...
    // PWR->CR1 |= PWR_CR1_VOS;                        // Voltage scaling =
    // range 1
    bits_assign(PWR->CR1, PWR_CR1_VOS, PWR_CR1_VOS_0);
    i = 1000;
    while (--i)
...
/*unsigned stm32_init_pll_clocking(struct stm32_pll_settings* s)
{
    ...
    RCC->PLLCFGR = (RCC_PLLCFGR_PLLSRC_HSE |
                    ...
                    | (RCC_PLLCFGR_PLLN & (s->Nkoeff << 6)) 		// 50 >=
PLLN >= 432 | (RCC_PLLCFGR_PLLP & (((s->Pkoeff >> 1) - 1) << 16)) |
(RCC_PLLCFGR_PLLQ & (s->Qkoeff << 24))
                   );
