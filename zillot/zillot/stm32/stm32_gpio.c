...
int stm32_gpio_setup(GPIO_TypeDef *g, unsigned int mask, uint32_t mode)
{
    GPIO_TypeDef *gpio = (GPIO_TypeDef *)(g);
    int mode_val = 0;
...
int stm32_gpio_set_alternate(GPIO_TypeDef *g, uint32_t mask, int32_t alternate)
{
    ...
    stm32_gpio_setup(g, mask, GPIO_MODE_ALTERNATE);
    return 0;
}

void stm32_gpio_set_maxspeed(GPIO_TypeDef *regs,
                             uint16_t map,
...
#elif defined CHIP_STM32F3XX || defined CHIP_STM32F4XX || defined STM32F4xx || \
    defined CHIP_STM32L4XX || defined STM32G4XX || defined STM32F7XX || defined STM32H7XX 
