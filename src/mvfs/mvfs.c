...
int vfs_write(struct file * filp, const char* data, unsigned int size, int flags)
{
	DTRACE();
	errno = 0;

	if (filp->node->n_ops->write == NULL)
		return ENOTSUP;

	return filp->node->n_ops->write(filp->node, filp, data, size, flags);
}

int vfs_read(struct file * filp, char* data, unsigned int size, int flags)
{
	DTRACE();

	if (filp->node->n_ops->read == NULL)
		return ENOTSUP;

	errno = 0;
	return filp->node->n_ops->read(filp->node, filp, data, size, flags);
}
