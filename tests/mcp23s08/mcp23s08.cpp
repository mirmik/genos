...
//	_useHaen == 1 ? writeuint8_t(IOCON, 0b00101000) : writeuint8_t(IOCON, 0b00100000);
	...
	//HERE
dprln("HERE");
	uint8_t init_iocon = readAddress(IOCON);
	DPRINTHEX(init_iocon);
	...
	digitalWrite(_cs, HIGH);
	writeuint8_t(IOCON, 0b00101000);
	//writeuint8_t(IOCON, 0b00001000);
	//writeuint8_t(IOCON, 0b00001000);
	//writeuint8_t(IOCON, 0b00000000);
	//writeuint8_t(IOCON, 0b00000000);
//	readAddress(GPIO);*/

	/*digitalWrite(_cs, LOW);
	SPI->exbyte(_writeCmd);
	SPI->exbyte(IODIR);	//set address pointer to first register
	SPI->exbyte(0xFF);				// reset first register
	for (uint8_t i = 0; i < OLAT; i++) {
		SPI->exbyte(0x04);			// reset other 10 registers
	}
	digitalWrite(_cs, HIGH);*/

	//gpioDigitalWrite(7,1);
	//gpioDigitalWrite(6,1);

	/*
	if (_useHaen){
		writeuint8_t(IOCON,0b00101000);//read datasheet for details!
	} else {
		writeuint8_t(IOCON,0b00100000);
	}
	*/
