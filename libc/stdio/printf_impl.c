...
after_flags:
		...
		case 'i':
			tmp.ulli = ops & OPS_LEN_MIN ? (signed char)va_arg(args, int)
					...
					: va_arg(args, int);
			pc += print_i(handler, printchar_data, tmp.ulli, 1, width,
					precision, ops, 10);
			break;
		case 'u':
		case 'o':
		...
		case 'X':
			...
			pc += print_i(handler, printchar_data, tmp.ulli, 0, width,
		...
		case 'A':
			tmp.ld = ops & OPS_LEN_LONGFP ? va_arg(args, long double)
					: va_arg(args, double);
			pc += print_f(handler, printchar_data, tmp.ld,
