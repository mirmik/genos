...
const ConstTransform4D Transform4D::identity = {{{1.0F, 0.0F, 0.0F, 0.0F},
                                                 {0.0F, 1.0F, 0.0F, 0.0F},
                                                 {0.0F, 0.0F, 1.0F, 0.0F},
                                                 {0.0F, 0.0F, 0.0F, 1.0F}}};

Matrix4D::Matrix4D(float n00,
                   float n01,
                   float n02,
                   float n03,
                   float n10,
...
Matrix4D::Matrix4D(const Bivector3D &a,
                   float n03,
                   const Bivector3D &b,
                   float n13,
                   const Bivector3D &c,
...
Matrix4D &Matrix4D::Set(float n00,
                        float n01,
                        float n02,
                        float n03,
                        float n10,
...
Matrix4D &Matrix4D::Set(const Vector4D &a,
                        const Vector4D &b,
                        const Vector4D &c,
                        const Vector4D &d)
{
...
Matrix4D Terathon::Inverse(const Matrix4D &m)
{
    ...
    vec_float invDet = VecSmearX(VecDivScalar(
        VecLoadScalarConstant<0x3F800000>(), VecDot3D(s, v) + VecDot3D(t, u)));
...
Transform4D::Transform4D(float n00,
                         float n01,
                         float n02,
                         float n03,
                         float n10,
...
Transform4D::Transform4D(const Bivector3D &r0,
                         float n03,
                         const Bivector3D &r1,
                         float n13,
                         const Bivector3D &r2,
...
Transform4D &Transform4D::Set(float n00,
                              float n01,
                              float n02,
                              float n03,
                              float n10,
...
Transform4D &Transform4D::Set(const Vector3D &a,
                              const Vector3D &b,
                              const Vector3D &c,
                              const Point3D &p)
{
...
Transform4D &Transform4D::SetMatrix3D(const Vector3D &a,
                                      const Vector3D &b,
                                      const Vector3D &c)
{
    m00 = a.x;
...
Transform4D Transform4D::MakeRotationX(float angle)
{
    ...
    return (Transform4D(
        1.0F, 0.0F, 0.0F, 0.0F, 0.0F, v.x, -v.y, 0.0F, 0.0F, v.y, v.x, 0.0F));
}
...
Transform4D Transform4D::MakeRotationY(float angle)
{
    ...
    return (Transform4D(
        v.x, 0.0F, v.y, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, -v.y, 0.0F, v.x, 0.0F));
}
...
Transform4D Transform4D::MakeRotationZ(float angle)
{
    ...
    return (Transform4D(
        v.x, -v.y, 0.0F, 0.0F, v.y, v.x, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F));
}
...
Transform4D Transform4D::MakeReflection(const Trivector4D &plane)
{
    ...
    return (Transform4D(x * plane.x + 1.0F,
                        nxny,
                        nxnz,
                        x * plane.w,
                        nxny,
...
{
    return (Transform4D(
        sx, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F));
}

Transform4D Transform4D::MakeScaleY(float sy)
{
    return (Transform4D(
        1.0F, 0.0F, 0.0F, 0.0F, 0.0F, sy, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F));
}

Transform4D Transform4D::MakeScaleZ(float sz)
{
    return (Transform4D(
        1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, sz, 0.0F));
}

Transform4D Transform4D::MakeScale(float scale)
{
    return (Transform4D(scale,
                        0.0F,
                        0.0F,
                        0.0F,
                        0.0F,
...
{
    return (Transform4D(
        sx, 0.0F, 0.0F, 0.0F, 0.0F, sy, 0.0F, 0.0F, 0.0F, 0.0F, sz, 0.0F));
}
...
Transform4D
Transform4D::MakeSkew(float angle, const Vector3D &a, const Vector3D &b)
{
    float t = Tan(angle);
    float x = a.x * t;
    float y = a.y * t;
...
Transform4D Terathon::operator*(const Transform4D &m1, const Transform4D &m2)
{
    ...
    VecStore(
        VecTransformPoint3D(a, b, c, VecLoad(&m1(0, 3)), VecLoad(&m2(0, 3))),
        &result(0, 3));
    ...
    return (Transform4D(
        ...
        m1(2, 0) * m2(0, 1) + m1(2, 1) * m2(1, 1) + m1(2, 2) * m2(2, 1),
        m1(2, 0) * m2(0, 2) + m1(2, 1) * m2(1, 2) + m1(2, 2) * m2(2, 2),
        m1(2, 0) * m2(0, 3) + m1(2, 1) * m2(1, 3) + m1(2, 2) * m2(2, 3) +
            m1(2, 3)));
...
Matrix4D Terathon::operator*(const Matrix4D &m1, const Transform4D &m2)
{
    ...
    return (Matrix4D(
        ...
        m1(3, 0) * m2(0, 1) + m1(3, 1) * m2(1, 1) + m1(3, 2) * m2(2, 1),
        m1(3, 0) * m2(0, 2) + m1(3, 1) * m2(1, 2) + m1(3, 2) * m2(2, 2),
        m1(3, 0) * m2(0, 3) + m1(3, 1) * m2(1, 3) + m1(3, 2) * m2(2, 3) +
            m1(3, 3)));
...
{
    return (Matrix4D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) +
...
Vector3D Terathon::operator*(const Transform4D &m, const Vector3D &v)
{
    ...
    VecStore3D(VecTransformVector3D(VecLoad(&m(0, 0)),
                                    VecLoad(&m(0, 1)),
                                    VecLoad(&m(0, 2)),
                                    VecLoadUnaligned(&v.x)),
               &result.x);
    return (result);
...
Point3D Terathon::operator*(const Transform4D &m, const Point3D &p)
{
    ...
    VecStore3D(VecTransformPoint3D(VecLoad(&m(0, 0)),
                                   VecLoad(&m(0, 1)),
                                   VecLoad(&m(0, 2)),
                                   VecLoad(&m(0, 3)),
                                   VecLoadUnaligned(&p.x)),
               &result.x);
    return (result);
...
{
    return (
        Vector2D(m(0, 0) * v.x + m(0, 1) * v.y, m(1, 0) * v.x + m(1, 1) * v.y));
}
...
{
    return (VecTransformVector3D(
        VecLoad(&m(0, 0)), VecLoad(&m(0, 1)), VecLoad(&m(0, 2)), v));
}

vec_float Terathon::TransformPoint3D(const Transform4D &m, vec_float p)
{
    return (VecTransformPoint3D(VecLoad(&m(0, 0)),
                                VecLoad(&m(0, 1)),
                                VecLoad(&m(0, 2)),
                                VecLoad(&m(0, 3)),
                                p));
...
Transform4D Terathon::Inverse(const Transform4D &m)
{
    ...
    vec_float invDet = VecSmearX(
        VecDivScalar(VecLoadScalarConstant<0x3F800000>(), VecDot3D(s, c)));
...
Vector3D Terathon::InverseTransform(const Transform4D &m, const Vector3D &v)
{
    ...
    return (
        Vector3D((b ^ c ^ v) * invDet, (c ^ a ^ v) * invDet, (s ^ v) * invDet));
}
...
Point3D Terathon::InverseTransform(const Transform4D &m, const Point3D &p)
{
    ...
    Vector3D q = p - d;
    return (
        Point3D((b ^ c ^ q) * invDet, (c ^ a ^ q) * invDet, (s ^ q) * invDet));
}
