...
namespace Terathon
{
    ...
    //# \ctor	template <typename type> explicit Vector2D(const Vec2D<type>&
    //v);
    //
    //# \param	a	The value of the <i>x</i> coordinate.
    //# \param	b	The value of the <i>y</i> coordinate.
    //# \param	v	Another 2D vector, possibly with a different component type,
    //that is converted to a $Vector2D$.
    //
    //# \desc
    //# The $Vector2D$ class is used to store a two-dimensional direction vector
    //# having floating-point components <i>x</i> and <i>y</i>. A direction
    //vector # stored in this class is assumed to have a <i>w</i> coordinate of
    //0 whenever it needs # to be converted to a four-dimensional
    //representation. Two-dimensional points # (for which the <i>w</i>
    //coordinate is 1) should be stored using the $@Point2D@$ class. # The
    //<i>z</i> coordinate of a 2D vector is always assumed to be 0.
    //#
    ...
    //vector. #				The value of $k$ must be 0 or 1.
    //
    ...
    //
    //# \action		bool operator ==(const Vector2D& v1, const Vector2D& v2)
    ...
    //
    //# \action		Vector2D operator +(const Vector2D& a, const Vector2D& b)
    //const; #				Returns the sum of the vectors $a$ and $b$.
    //
    //# \action		Vector2D operator -(const Vector2D& a, const Vector2D& b)
    //const; #				Returns the difference of the vectors $a$ and $b$.
    //
    ...
    //
    //# \action		Vector2D operator *(const Vector2D& a, const Vector2D& b)
    //const; #				Returns the componentwise product of the vectors $a$
    //and $b$.
    //
    ...
    //
    //# \privbase	Vec2D	Vectors use a generic base class to store their
    //components.
    //
    //# \also	$@Point2D@$
    //# \also	$@Vector4D@$
    ...
    //
    //# \desc
    //# The $Set$ function sets the <i>x</i> and <i>y</i> coordinates of a
    //vector to # the values given by the $a$ and $b$ parameters, respectively.
    //#
    ...
    //
    //# \desc
    //# The $Normalize$ function multiplies a 2D vector by the inverse of its
    //magnitude, # normalizing it to unit length. Normalizing the zero vector
    //produces undefined results.
    //#
    //# The return value is a reference to the vector object.

    //# \function	Vector2D::Rotate		Rotates a vector in the <i>x</i>-<i>y</i>
    //plane.
    //
    ...
    //
    //# \desc
    //# The $Rotate$ function rotates a vector in the <i>x</i>-<i>y</i> plane
    ...
    public:
        ...
        explicit Vector2D(const Vec2D<type> &v)
            : Vec2D<TypeVector2D>(float(v.x), float(v.y))
        {
        }
        ...
        Vector2D &
        operator=(const Subvec2D<type_struct, count, index_x, index_y> &v)
        {
            xy = v;
            return *this;
        }

        template <typename type> Vector2D &operator=(const Vec2D<type> &v)
        {
            x = float(v.x);
            y = float(v.y);
            return (*this);
    ...
    operator+(const Vector2D &a,
              const Subvec2D<type_struct, count, index_x, index_y> &b)
    {
    ...
    //
    //# \desc
    //# The $Point2D$ class is used to store a two-dimensional point having
    //floating-point # coordinates <i>x</i> and <i>y</i>. The difference between
    //a point and a # vector is that a point is assumed to have a <i>w</i>
    //coordinate of 1 whenever it # needs to be converted to a four-dimensional
    //representation, whereas a vector is # assumed to have a <i>w</i>
    //coordinate of 0. Such a conversion occurs when a vector or # point is
    //assigned to a $@Vector4D@$ object or is multiplied by a $@Transform4D@$
    //object.
    //#
    //# The default constructor leaves the components of the vector undefined.
    //# If the values $a$ and $b$ are supplied, then they are assigned to the
    //# <i>x</i> and <i>y</i> coordinates of the base vector object,
    //respectively.
    //#
    //# The difference between two points produces a direction vector. A
    //two-dimensional # direction vector is converted to a point by adding it to
    //the identifier $Zero2D$.
    //
    ...
    //$p$.
    //
    //# \base	Vector2D	A $Point2D$ object behaves much like a $@Vector2D@$
    ...
    public:
        ...
        template <typename type> Point2D &operator=(const Vec2D<type> &v)
        {
            x = float(v.x);
            y = float(v.y);
            return (*this);
    ...
    operator+(const Point2D &p,
              const Subvec2D<type_struct, count, index_x, index_y> &v)
    {
    ...
    inline const Point2D &operator+(const Origin2D &,
                                    const Subvec2D<type_struct, count, 0, 1> &v)
    {
    ...
    template <typename type_struct, int count>
    inline const Point2D &operator+(const Origin2D &,
                                    const Subvec2D<type_struct, count, 1, 2> &v)
    {
    ...
    template <typename type_struct, int count>
    inline const Point2D &operator+(const Origin2D &,
                                    const Subvec2D<type_struct, count, 2, 3> &v)
    {
