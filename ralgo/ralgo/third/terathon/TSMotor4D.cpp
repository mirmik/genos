...
const ConstMotor4D Motor4D::identity = {
...
Point3D Motor4D::GetPosition(void) const
{
    ...
    return (
        Point3D((A03 + B03) * 2.0F, (A13 + B13) * 2.0F, (A23 + B23) * 2.0F));
}
...
{
    return (Motor4D(a.rotor.w * b.rotor.x + a.rotor.x * b.rotor.w +
                        a.rotor.y * b.rotor.z - a.rotor.z * b.rotor.y,
                    a.rotor.w * b.rotor.y + a.rotor.y * b.rotor.w +
                        a.rotor.z * b.rotor.x - a.rotor.x * b.rotor.z,
                    a.rotor.w * b.rotor.z + a.rotor.z * b.rotor.w +
                        a.rotor.x * b.rotor.y - a.rotor.y * b.rotor.x,
                    a.rotor.w * b.rotor.w - a.rotor.x * b.rotor.x -
                        a.rotor.y * b.rotor.y - a.rotor.z * b.rotor.z,
                    a.screw.w * b.rotor.z + a.screw.x * b.rotor.y -
                        a.screw.y * b.rotor.x + a.screw.z * b.rotor.w +
                        b.screw.w * a.rotor.z - b.screw.x * a.rotor.y +
                        b.screw.y * a.rotor.x + b.screw.z * a.rotor.w,
                    a.screw.w * b.rotor.x + a.screw.x * b.rotor.w +
                        a.screw.y * b.rotor.z - a.screw.z * b.rotor.y +
                        b.screw.w * a.rotor.x + b.screw.x * a.rotor.w -
                        b.screw.y * a.rotor.z + b.screw.z * a.rotor.y,
                    a.screw.w * b.rotor.y - a.screw.x * b.rotor.z +
                        a.screw.y * b.rotor.w + a.screw.z * b.rotor.x +
                        b.screw.w * a.rotor.y + b.screw.x * a.rotor.z +
                        b.screw.y * a.rotor.w - b.screw.z * a.rotor.x,
                    a.screw.w * b.rotor.w - a.screw.x * b.rotor.x -
                        a.screw.y * b.rotor.y - a.screw.z * b.rotor.z +
                        b.screw.w * a.rotor.w - b.screw.x * a.rotor.x -
                        b.screw.y * a.rotor.y - b.screw.z * a.rotor.z));
}

Motor4D Terathon::operator*(const Motor4D &Q, const Quaternion &r)
{
    return (Motor4D(
        Q.rotor.w * r.x + Q.rotor.x * r.w + Q.rotor.y * r.z - Q.rotor.z * r.y,
...
{
    return (Motor4D(
        r.w * Q.rotor.x + r.x * Q.rotor.w + r.y * Q.rotor.z - r.z * Q.rotor.y,
...
Point3D Terathon::Transform(const Point3D &p, const Motor4D &Q)
{
    ...
    a = VecMadd(VecShuffle<3, 0, 0, 1>(q, q),
                s1,
                VecShuffle<3, 1, 2, 2>(q, q) * s2) +
        a;
    a = VecNmsub(
        VecShuffle<3, 0, 2, 1>(r2, r2) + VecShuffle<3, 1, 0, 2>(r2, r2), q, a);
    ...
    return (Point3D(p.x + (ux * rw - rx * uw + (ry * v.z - rz * v.y) +
                           p.y * rxry + p.z * rzrx - p.x * (ry2 + rz2)) *
                              2.0F,
                    p.y + (uy * rw - ry * uw + (rz * v.x - rx * v.z) +
                           p.x * rxry + p.z * ryrz - p.y * (rz2 + rx2)) *
                              2.0F,
                    p.z + (uz * rw - rz * uw + (rx * v.y - ry * v.x) +
                           p.x * rzrx + p.y * ryrz - p.z * (rx2 + ry2)) *
                              2.0F));
...
Bivector4D Terathon::Transform(const Bivector4D &L, const Motor4D &Q)
{
    ...
    return (
        Bivector4D(vx + ((ryrw * vz - rzrw * vy) + vy * rxry + vz * rzrx -
                         vx * (ry2 + rz2)) *
                            2.0F,
                   vy + ((rzrw * vx - rxrw * vz) + vx * rxry + vz * ryrz -
                         vy * (rz2 + rx2)) *
                            2.0F,
                   vz + ((rxrw * vy - ryrw * vx) + vx * rzrx + vy * ryrz -
                         vz * (rx2 + ry2)) *
                            2.0F,
                   mx + (mz * (ryrw + rzrx) - my * (rzrw + rxry) +
                         vy * (ux * ry + uy * rx - uz * rw - rz * uw) +
                         vz * (rz * ux + rw * uy + rx * uz + ry * uw) -
                         vx * (uy * ry - uz * rz) * 2.0F - mx * (ry2 + rz2)) *
                            2.0F,
                   my + (mx * (rzrw + rxry) - mz * (rxrw + ryrz) +
                         vz * (uy * rz + uz * ry - ux * rw - rx * uw) +
                         vx * (ry * ux + rx * uy + rw * uz + rz * uw) -
                         vy * (ux * rx - uz * rz) * 2.0F - my * (rz2 + rx2)) *
                            2.0F,
                   mz + (my * (rxrw + ryrz) - mx * (ryrw + rzrx) +
                         vx * (uz * rx + ux * rz - uy * rw - ry * uw) +
                         vy * (rw * ux + rz * uy + ry * uz + rx * uw) -
                         vz * (ux * rx - uy * ry) * 2.0F - mz * (rx2 + ry2)) *
                            2.0F));
}
...
Trivector4D Terathon::Transform(const Trivector4D &f, const Motor4D &Q)
{
    ...
    return (Trivector4D(f.x + ((ry * f.z - rz * f.y) * rw + f.y * rxry +
                               f.z * rzrx - f.x * (ry2 + rz2)) *
                                  2.0F,
                        f.y + ((rz * f.x - rx * f.z) * rw + f.x * rxry +
                               f.z * ryrz - f.y * (rz2 + rx2)) *
                                  2.0F,
                        f.z + ((rx * f.y - ry * f.x) * rw + f.x * rzrx +
                               f.y * ryrz - f.z * (rx2 + ry2)) *
                                  2.0F,
                        f.w + (fr * uw - fu * rw + f.x * (ry * uz - rz * uy) +
                               f.y * (rz * ux - rx * uz) +
                               f.z * (rx * uy - ry * ux)) *
                                  2.0F));
}
