...
namespace Terathon
{
    ...
    //
    //# \desc
    //# The $Bivector3D$ class is used to store a three-dimensional bivector
    //having three floating-point components # named $x$, $y$, and $z$.
    //#
    ...
    //<i>z</i> coordinates set to 1, giving a representation of the 2D line
    //containing # both points. If the point $p$ and the direction $v$ are
    //specified, then the line contains the point $p$ and runs # parallel to the
    //direction $v$.
    //
    ...
    //
    //# \action		Bivector3D operator +(const Bivector3D& a, const Bivector3D&
    //b); #				Returns the sum of the bivectors $a$ and $b$.
    //
    //# \action		Bivector3D operator -(const Bivector3D& a, const Bivector3D&
    //b); #				Returns the difference of the bivectors $a$ and $b$.
    //
    ...
    //
    //# \action		Bivector3D operator /(const Bivector3D& v, float s);
    //#				Returns the product of the bivector $v$ and the inverse of the
    //scalar $s$.
    //
    //# \action		bool operator ==(const Bivector3D& a, const Bivector3D& b);
    //#				Returns a boolean value indicating whether the two bivectors $a$
    ...
    //#				Returns the wedge product of the 2D points $p$ and $q$. The
    //<i>z</i> coordinates of $p$ and $q$ are assumed to be 1.
    //
    //# \action		Bivector3D operator ^(const Point2D& p, const Vector2D& v);
    //#				Returns the wedge product of the 2D point $p$ and the 2D vector
    //$v$. The <i>z</i> coordinate of $p$ is assumed to be 1.
    //
    //# \action		Vector3D operator ^(const Bivector3D& a, const Bivector3D&
    //b); #				Returns the antiwedge product of the bivectors $a$ and
    //$b$.
    //
    //# \action		float operator ^(const Bivector3D& a, const Vector3D& b);
    //#				Returns the antiwedge product of the bivector $a$ and the vector
    //$b$.
    //
    //# \action		float operator ^(const Vector3D& a, const Bivector3D& b);
    //#				Returns the antiwedge product of the vector $a$ and the bivector
    //$b$.
    //
    //# \action		Vector3D Project(const Vector3D& a, const Bivector3D& b);
    //#				Returns
    //(<b>b&#x0332;</b>&#x202F;&#x2227;&#x202F;<b>a</b>)&#x202F;&#x2228;&#x202F;<b>b</b>,
    //which is the projection of $a$ onto $b$ under the assumption that the
    //magnitude of $b$ is one.
    //
    //# \privbase	Antivec3D	Antivectors use a generic base class to store
    //their components.
    //
    //# \also	$@Vector3D@$
    //# \also	$@Vector2D@$
    ...
    //
    //# \desc
    //# The $Set$ function replaces all three components of a bivector with new
    //values.
    //#
    //# If points $p$ and $q$ are specified, then the bivector is initialized to
    //the wedge product between homogeneous # extensions of $p$ and $q$ with
    ...
    public:
        ...
        Bivector3D(float a, float b, float c)
            : Antivec3D<TypeBivector3D>(a, b, c)
        {
        }

        template <typename type>
        explicit Bivector3D(const Antivec3D<type> &v)
            : Antivec3D<TypeBivector3D>(float(v.x), float(v.y), float(v.z))
        {
        }
        ...
        template <typename type_struct,
                  ...
                  int index_z>
        Bivector3D &operator=(
            const Subvec3D<type_struct, true, count, index_x, index_y, index_z>
                &v)
        {
            xyz = v;
            return (*this);
    ...
    {
        return (Bivector3D(a.y * b.z - a.z * b.y,
                           a.z * b.x - a.x * b.z,
                           a.x * b.y - a.y * b.x));
    }
