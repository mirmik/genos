...
Motor4D Terathon::operator*(const Flector4D &a, const Flector4D &b)
{
    ...
    return (Motor4D(fz * gy - fy * gz - fx * qw - pw * gx,
                    ...
                    pz * gy - py * gz + fy * qz - fz * qy + fx * gw - fw * gx +
                        pw * qx - px * qw,
                    px * gz - pz * gx + fz * qx - fx * qz + fy * gw - fw * gy +
                        pw * qy - py * qw,
                    py * gx - px * gy + fx * qy - fy * qx + fz * gw - fw * gz +
                        pw * qz - pz * qw,
                    px * gx + py * gy + pz * gz + pw * gw - fx * qx - fy * qy -
                        fz * qz - fw * qw));
}
...
Flector4D Terathon::operator*(const Flector4D &a, const Motor4D &b)
{
    ...
    return (Flector4D(hz * uy - hy * uz + hw * rx - hx * uw + sy * rz -
                          sz * ry + sx * rw - sw * ux,
                      hx * uz - hz * ux + hw * ry - hy * uw + sz * rx -
                          sx * rz + sy * rw - sw * uy,
                      hy * ux - hx * uy + hw * rz - hz * uw + sx * ry -
                          sy * rx + sz * rw - sw * uz,
                      sw * rw - hx * rx - hy * ry - hz * rz,
                      hy * rz - hz * ry + sw * rx + hx * rw,
                      hz * rx - hx * rz + sw * ry + hy * rw,
                      hx * ry - hy * rx + sw * rz + hz * rw,
                      hw * rw + hx * ux + hy * uy + hz * uz - sw * uw -
                          sx * rx - sy * ry - sz * rz));
}
...
Flector4D Terathon::operator*(const Motor4D &a, const Flector4D &b)
{
    ...
    return (Flector4D(hz * uy - hy * uz + hw * rx + hx * uw + sz * ry -
                          sy * rz + sx * rw + sw * ux,
                      hx * uz - hz * ux + hw * ry + hy * uw + sx * rz -
                          sz * rx + sy * rw + sw * uy,
                      hy * ux - hx * uy + hw * rz + hz * uw + sy * rx -
                          sx * ry + sz * rw + sw * uz,
                      sw * rw - hx * rx - hy * ry - hz * rz,
...
Vector3D Terathon::Transform(const Vector3D &v, const Flector4D &G)
{
    ...
    return (Vector3D(
        v.x + ((hzsw - hxhy) * v.y - (hzhx + hysw) * v.z - (hx2 + sw2) * v.x) *
                  2.0F,
        v.y + ((hxsw - hyhz) * v.z - (hxhy + hzsw) * v.x - (hy2 + sw2) * v.y) *
                  2.0F,
        v.z + ((hysw - hzhx) * v.x - (hyhz + hxsw) * v.y - (hz2 + sw2) * v.z) *
                  2.0F));
}
...
Point3D Terathon::Transform(const Point3D &p, const Flector4D &G)
{
    ...
    return (Point3D(
        p.x + ((hzsw - hxhy) * p.y - (hzhx + hysw) * p.z - (hx2 + sw2) * p.x +
               sx * sw - hx * hw + hy * sz - hz * sy) *
                  2.0F,
        p.y + ((hxsw - hyhz) * p.z - (hxhy + hzsw) * p.x - (hy2 + sw2) * p.y +
               sy * sw - hy * hw + hz * sx - hx * sz) *
                  2.0F,
        p.z + ((hysw - hzhx) * p.x - (hyhz + hxsw) * p.y - (hz2 + sw2) * p.z +
               sz * sw - hz * hw + hx * sy - hy * sx) *
                  2.0F));
}
...
Bivector4D Terathon::Transform(const Bivector4D &L, const Flector4D &G)
{
    ...
    return (Bivector4D(
        vx +
            ((hxhy - hzsw) * vy + (hzhx + hysw) * vz - (hy2 + hz2) * vx) * 2.0F,
        vy +
            ((hyhz - hxsw) * vz + (hxhy + hzsw) * vx - (hz2 + hx2) * vy) * 2.0F,
        vz +
            ((hzhx - hysw) * vx + (hyhz + hxsw) * vy - (hx2 + hy2) * vz) * 2.0F,
        mx + ((hx * sy + hy * sx - sz * sw - hz * hw) * vy +
              (hx * sz + hz * sx + sy * sw + hy * hw) * vz -
              (hy * sy + hz * sz) * vx * 2.0F + (hzsw - hxhy) * my -
              (hzhx + hysw) * mz - (hx2 + sw2) * mx) *
                 2.0F,
        my + ((hy * sz + hz * sy - sx * sw - hx * hw) * vz +
              (hy * sx + hx * sy + sz * sw + hz * hw) * vx -
              (hz * sz + hx * sx) * vy * 2.0F + (hxsw - hyhz) * mz -
              (hxhy + hzsw) * mx - (hy2 + sw2) * my) *
                 2.0F,
        mz + ((hz * sx + hx * sz - sy * sw - hy * hw) * vx +
              (hz * sy + hy * sz + sx * sw + hx * hw) * vy -
              (hx * sx + hy * sy) * vz * 2.0F + (hysw - hzhx) * mx -
              (hyhz + hxsw) * my - (hz2 + sw2) * mz) *
                 2.0F));
}
...
Trivector4D Terathon::Transform(const Trivector4D &f, const Flector4D &G)
{
    ...
    return (Trivector4D(
        f.x + ((hzsw - hxhy) * f.y - (hzhx + hysw) * f.z - (hx2 + sw2) * f.x) *
                  2.0F,
        f.y + ((hxsw - hyhz) * f.z - (hxhy + hzsw) * f.x - (hy2 + sw2) * f.y) *
                  2.0F,
        f.z + ((hysw - hzhx) * f.x - (hyhz + hxsw) * f.y - (hz2 + sw2) * f.z) *
                  2.0F,
        f.w + ((sx * sw - hx * hw - hy * sz + hz * sy) * f.x +
               (sy * sw - hy * hw - hz * sx + hx * sz) * f.y +
               (sz * sw - hz * hw - hx * sy + hy * sx) * f.z) *
                  2.0F));
}
