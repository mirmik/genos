...
namespace Terathon
{
    ...
    //# \ctor	Trivector4D(const Point3D& p1, const Point3D& p2, const Point3D&
    //p3);
    //
    ...
    //# \param	n			A 3D bivector whose entries are copied to the <i>x</i>,
    ...
    //<i>w</i>.
    //#
    //# The default constructor leaves the components of the trivector
    //undefined. # If the values $a$, $b$, $c$, and $d$ are supplied, then they
    //are assigned to the # <i>x</i>, <i>y</i>, <i>z</i>, and <i>w</i>
    //coordinates of the trivector, respectively.
    //#
    //# If the $p$ parameter is specified, then the <i>w</i> coordinate is given
    //by &minus;($n$&#x202F;&and;&#x202F;$p$). # If the $p1$, $p2$, and $p3$
    //parameters are specified, then the trivector is set to the # triple wedge
    //product $p1$&#x202F;&and;&#x202F;$p2$&#x202F;&and;&#x202F;$p3$.
    //
    //# \operator	float& operator [](machine k);
    //#				Returns a reference to the $k$-th scalar component of a
    //trivector. #				The value of $k$ must be 0, 1, 2, or 3.
    //
    ...
    //
    //# \action		bool operator ==(const Trivector4D& a, const Trivector4D&
    //b);
    //#				Returns a boolean value indicating whether the two trivectors $a$
    //and $b$ are equal.
    //
    //# \action		bool operator !=(const Trivector4D& a, const Trivector4D&
    //b);
    ...
    //#				Returns the antireverse of the trivector $v$ (which is just the
    //trivector $v$ itself).
    //
    ...
    //
    //# \action		Trivector4D operator /(const Trivector4D& v, float s);
    //#				Returns the product of the trivector $v$ and the inverse of the
    //scalar $s$.
    //
    //# \action		float operator ^(const Vector4D& v, const Trivector4D& f);
    //#				Returns the antiwedge product of the vector $v$ and the trivector
    //$f$.
    //
    //# \action		float operator ^(const Vector3D& v, const Trivector4D& f);
    //#				Returns the antiwedge product of the 3D vector $v$ and the
    //trivector $f$. The <i>w</i> coordinate of $v$ is assumed to be 0.
    //
    //# \action		float operator ^(const Point3D& p, const Trivector4D& v);
    //#				Returns the antiwedge product of the 3D point $p$ and the
    //trivector $f$. The <i>w</i> coordinate of $p$ is assumed to be 1. #
    //This gives the distance from a unitized plane represented by a
    //$Trivector4D$ object to the point $p$.
    //
    //# \action		float operator ^(const Point2D& p, const Trivector4D& f);
    //#				Returns the antiwedge product of the 2D point $p$ and the
    //trivector $f$. The <i>z</i> coordinate of $p$ is assumed to be 0, and the
    ...
    //#				Returns the projection of the point $p$ onto the plane $f$ under
    //the assumption that the plane is unitized.
    //
    //# \action		Trivector4D Antiproject(const Trivector4D& f, const Point3D&
    //p);
    //#				Returns the antiprojection of the plane $f$ onto the point $p$
    //(where $p$ is always unitized because it has an implicit <i>w</i>
    //coordinate of 1).
    //
    //# \privbase	Antivec4D	Antivectors use a generic base class to store
    //their components.
    //
    //# \also	$@Vector4D@$
    //# \also	$@Point3D@$
    ...
    //# \function	Trivector4D::Set		Sets all four components of a
    //trivector.
    //
    ...
    //# \proto	Trivector4D& Set(const Point3D& p1, const Point3D& p2, const
    //Point3D& p3);
    //
    ...
    //coordinates of a trivector to # the values given by the $a$, $b$, $c$, and
    ...
    //# \function	Trivector4D::Unitize		Unitizes the weight of a 4D
    //trivector.
    //
    //# \proto	Trivector4D& Unitize(void);
    //
    //# \desc
    //# The $Unitize$ function multiplies a 4D trivector by the inverse
    //magnitude of its weight, which is the # 3D bivector given by its <i>x</i>,
    //<i>y</i>, and <i>z</i> coordinates. After calling this function, the plane
    //# has a unit-length normal. If the <i>x</i>, <i>y</i>, and <i>z</i>
    //coordinates are all zero, then the result is undefined.
    //#
    //# The return value is a reference to the trivector object.
    ...
    public:
        ...
        Trivector4D(float a, float b, float c, float d)
            : Antivec4D<TypeTrivector4D>(a, b, c, d)
        {
        }
        ...
        Trivector4D &
        Set(const Point3D &p1, const Point3D &p2, const Point3D &p3)
        {
            xyz = (p2 - p1) ^ (p3 - p1);
            w = -(xyz ^ p1);
            return (*this);
    ...
    template <typename type_struct,
              ...
              int index_z,
              int index_w>
    inline float operator^(const Trivector4D &a,
    ...
    inline float operator^(
        const Trivector4D &a,
        const Subvec3D<type_struct, false, count, index_x, index_y, index_z> &b)
    {
        return (a.xyz ^ b);
    }
    ...
    template <typename type_struct,
              ...
              int index_z,
              int index_w>
    inline float Antiwedge(const Trivector4D &a,
                           const Subvec4D<type_struct,
                                          false,
                                          count,
                                          index_x,
