...
namespace Terathon
{
    ...
    //
    //# \ctor	Matrix3D();
    //# \ctor	Matrix3D(float n00, float n01, float n02, float n10, float n11,
    //float n12, float n20, float n21, float n22); # \ctor	Matrix3D(const
    //Vector3D& a, const Vector3D& b, const Vector3D& c);
    //
    ...
    //
    //# \desc
    //# The $Matrix3D$ class is used to store a 3&#x202F;&times;&#x202F;3
    ...
    //
    //# \action		Matrix3D operator /(const Matrix3D& m, float s);
    //#				Returns the product of the matrix $m$ and the inverse of the
    //scalar $s$.
    //
    ...
    //#				Returns the product of the row vector $v$ and the matrix
    //$m$.
    //
    //# \action		Bivector3D operator *(const Bivector3D& v, const Matrix3D&
    //m);
    //#				Returns the product of the row bivector $v$ and the matrix
    //$m$.
    //
    ...
    //
    //# \privbase	Mat3D	Matrices use a generic base class to store their
    //components.
    //
    //# \also	$@Vector3D@$
    //# \also	$@Matrix4D@$
    ...
    //
    //# \proto	Matrix3D& Set(float n00, float n01, float n02, float n10, float
    //n11, float n12, float n20, float n21, float n22); # \proto	Matrix3D&
    //Set(const Vector3D& a, const Vector3D& b, const Vector3D& c);
    //
    ...
    //
    //# \desc
    //# The $Set$ function sets all nine entries of a matrix, either by
    //specifying each entry individually # or by specifying each of the three
    //columns as 3D vectors.
    //#
    ...
    //# \function	Matrix3D::SetIdentity		Sets a matrix to the
    ...
    //# \function	Matrix3D::Orthogonalize		Orthogonalizes the columns of a
    //matrix.
    //
    //# \proto	Matrix3D& Orthogonalize(column);
    //
    //# \param	column		The index of the column whose direction does not
    //change. This must be 0, 1, or 2.
    //
    //# \desc
    //# The $Orthogonalize$ function uses Gram-Schmidt orthogonalization to
    //orthogonalize the columns # of a matrix. The column whose index is
    //specified by the $column$ parameter is normalized to unit length. # The
    //remaining two columns are orthogonalized and made unit length. Only the
    //two columns not specified # by the $column$ parameter can change
    //direction.

    //# \function	Matrix3D::MakeRotationX		Returns a matrix that represents a
    //rotation about the <i>x</i> axis.
    //
    ...
    //# \function	Matrix3D::MakeRotationY		Returns a matrix that represents a
    //rotation about the <i>y</i> axis.
    //
    ...
    //# \function	Matrix3D::MakeRotationZ		Returns a matrix that represents a
    //rotation about the <i>z</i> axis.
    //
    ...
    //# \function	Matrix3D::MakeRotation		Returns a matrix that represents a
    //rotation about a given axis.
    //
    //# \proto	static Matrix3D MakeRotation(float angle, const Bivector3D&
    //axis);
    //
    //# \param	angle	The angle through which to rotate, in radians.
    ...
    //# \function	Matrix3D::MakeScale		Returns a matrix that represents a
    //scale.
    //
    ...
    //# If only the $scale$ parameter is specified, then the scale is uniform
    //along all three axes. # If the $sx$, $sy$, and $sz$ parameters are
    //specified, then they correspond to the scale along # the <i>x</i>,
    //<i>y</i>, and <i>z</i> axis, respectively.
    //
    //# \also	$@Matrix3D::SetIdentity@$
    //# \also	$@Matrix3D::Set@$
    ...
    public:
        ...
        TERATHON_API Matrix3D(float n00,
                              float n01,
                              float n02,
                              float n10,
                              float n11,
        ...
        TERATHON_API Matrix3D &Set(float n00,
                                   float n01,
                                   float n02,
                                   float n10,
                                   float n11,
        ...
        template <typename type_struct,
                  ...
                  int index_21,
                  int index_22>
        Matrix3D &operator=(const Submat3D<type_struct,
                                           count,
                                           index_00,
                                           index_01,
                                           index_02,
        ...
        TERATHON_API static Matrix3D MakeRotation(float angle,
    ...
    template <typename type_struct,
              ...
              int index_y,
              int index_z>
    inline typename type_struct::vector3D_type operator*(
        const Matrix3D &m,
    ...
    template <typename type_struct,
              ...
              int index_21,
              int index_22>
    inline Vector3D operator*(const Submat3D<type_struct,
                                             count,
                                             index_00,
                                             index_01,
                                             index_02,
    ...
    template <typename type_struct,
              ...
              int index_21,
              int index_22>
    inline Matrix3D operator*(const Matrix3D &m1,
                              const Submat3D<type_struct,
                                             count,
                                             index_00,
                                             index_01,
    ...
    template <typename type_struct,
              ...
              int index_21,
              int index_22>
    inline Matrix3D operator*(const Submat3D<type_struct,
                                             count,
                                             index_00,
                                             index_01,
                                             index_02,
