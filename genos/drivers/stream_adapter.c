...
int stream_adapter_write(struct file_head *head,
                         const void *data,
                         unsigned int size)
{
    struct stream_adapter *adapter =
        mcast_out(head, struct stream_adapter, dev.fil);
...
int stream_adapter_read(struct file_head *head, void *data, unsigned int size)
{
    struct stream_adapter *adapter =
        mcast_out(head, struct stream_adapter, dev.fil);
...
void stream_adapter_init(struct stream_adapter *adapter,
                         const char *name,
                         const struct stream_adapter_operations *ops)
{
    ...
    tasklet_init(
        &adapter->checker, stream_adapter_tasklet_routine, (void *)adapter);
