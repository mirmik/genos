...
int devices_namespace_lookup(struct namespace_head * ns, struct file_head ** filpp, const char * name) 
{
	const char * devname = path_remove_prefix(name, ns->path);

	struct device_head * head;
	dlist_for_each_entry(head, &devices_list, lnk_devices_list) 
	{
		if (strcmp(devname, head->name) == 0) 
		{
			*filpp = &head->fil;
			return 0;
		}
