...
ввода в составе какого-либо процесса обработчика, или в качестве самостоятельного
обработчика, если система не оперирует процессами.

Компонент основан на машине состояний igris/shell/readline.
*/

#ifndef GENOS_VTERM_H
#define GENOS_VTERM_H
...
static inline 
void vterm_set_write_callback(
	struct vterm_automate * vterm, vterm_write_t callback, void * privdata) 
{
...
static inline 
void vterm_set_execute_callback(
	struct vterm_automate * vterm, vterm_execute_t callback, void * privdata) 
{
...
static inline 
void vterm_set_signal_callback(
	struct vterm_automate * vterm, vterm_signal_t callback, void * privdata) 
{
	vterm->signal_callback = callback;
	vterm->signal_privdata = privdata;
}
static inline
void vterm_automate_init_step(struct vterm_automate * vterm) 
{
